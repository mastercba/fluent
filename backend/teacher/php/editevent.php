...
//Recupero variables
	$event_id3 = $_GET['eventid'];
	$_SESSION['eventid_pencil'] = $event_id3;
...
//-----------------------------------recupero variables de la db a editar-------------------------------
	//Conexion
		...
		while($row = mysql_fetch_array($result)){
			if($event_id3 == $row['id']){
				$old_idstudent = $row['idstudent'];
					$_SESSION['old_idstudent'] = $old_idstudent;
				$old_idteacher = $row['idteacher'];
					$_SESSION['old_idteacher'] = $old_idteacher;
				$old_idmedia = $row['idmedia'];
					$_SESSION['old_idmedia'] = $old_idmedia;
				$old_idstatus = $row['idstatus']; 
					$_SESSION['old_idstatus'] = $old_idstatus;
				$old_hrs = $row['hrs'];
					$_SESSION['old_hrs'] = $old_hrs;
				$old_idunit = $row['idunit'];
					$_SESSION['old_idunit'] = $old_idunit;					
				$old_event_date = $row['event_date'];
					$_SESSION['old_event_date'] = $old_event_date;
				$old_event_time = $row['event_time'];
					$_SESSION['old_event_time'] = $old_event_time;
			}else{}
		}
	//cerrar
		mysql_close($conex);
			   ...
			   do{
				if($event['idstudent']== $student['id']){
					...
					//recupera el nombre de la unidad
						$query0 = "select * from events"; 
						$result0 = mysql_query($query0);
						while($row0 = mysql_fetch_array($result0)){
							if($row0['id'] == $eventid){
								$query01 = "select * from units"; 
								$result01 = mysql_query($query01);
									while($row01 = mysql_fetch_array($result01)){
										if($row01['id'] == $row0['idunit']){
											$unite = $row01['unit'];
											//// aqui session de la unidad 2 call
											////$_SESSION['go2unit'] = $unite;
										}else{}
									}
							}else{}
						}
///////AQUI pregunta para editar esa fila!//////////////////////////////////////////////////////////////
					if($event['id'] == $event_id3){
						$_SESSION['eventid4']=$event['id'];
						echo"<tr>
						<form action='saveedit.php' method = 'POST'>

						<td>".$snombre." ".$sapellido."</td>
						
						<td><input type='date' name='evnt_date' value='".$old_event_date."' size=5>
						<input type='time' name='evnt_time' value='".$old_event_time."' size=5></td>
						

						<td><input type='number' name='nehrs' min='15' max='90' step='15' value='".$old_hrs."' size=4></td>
						<td><select name=neidstatus>
							<option value = '".$old_idstatus."'>".$old_idstatus."</option>
							<option>     </option>
			    				<option>SCHEDULE</option>
			    				<option>DONE</option>
			    				<option>CCL</option>
						...
						<td>".$email."</td>
						<td><select name=neidmedia>
							<option value = '".$old_idmedia."'>".$old_idmedia."</option>
							<option>     </option>
			    				<option>SKYPE</option>
			    				<option>PHONE</option>
			    				<option>nPHONE</option>
...
if($status == "DONE"){
		if($event['report']== 0){
	echo"<tr><td>".$snombre." ".$sapellido."</td><td>".$event['event_date']." | ".$event['event_time']."</td>
	<td>".$hrs."</td><td>".$status."</td><td>".$email."</td><td>".$media."</td><td><img src='../images/pdf.png' width='14' height=14'><s>".$unite."</s></td>
	<td><a href='#?eventid=".$eventid."'><img src='../images/login-icon.png' alt='edit' width='14' height=14'></a>
	|<a href='editevent.php?eventid=".$eventid."'><img src='../images/pencil.png' alt='edit' width='14' height=14'></a>
	|<a href = 'deleteevent.php?eventid=".$eventid."'><img src='../images/trash.png' width='14' height=14'></a>
	|<a href = 'reportevent.php?eventid=".$eventid."'><img src='../images/sign.gif' width='14' height=14'></a>
	</td><td></tr>";
		}else{
	echo"<tr><td>".$snombre." ".$sapellido."</td><td>".$event['event_date']." | ".$event['event_time']."</td>
	<td>".$hrs."</td><td>".$status."</td><td>".$email."</td><td>".$media."</td><td><img src='../images/pdf.png' width='14' height=14'><s>".$unite."</s></td>
	...
	|<a href='editevent.php?eventid=".$eventid."'><img src='../images/pencil.png' alt='edit' width='14' height=14'></a>
	|<a href = 'deleteevent.php?eventid=".$eventid."'><img src='../images/trash.png' width='14' height=14'></a>
	|<a href = 'editreport.php?eventid=".$eventid."'><img src='../images/report.png' width='14' height=14'></a>
	</td><td></tr>";
		}
}else{
	//prepara la variable $unidos = $eventid & $unite
	$unidos = $unite." ".$eventid;
	echo"<tr><td>".$snombre." ".$sapellido."</td><td>".$event['event_date']." | ".$event['event_time']."</td>
	<td>".$hrs."</td><td>".$status."</td><td>".$email."</td><td>".$media."</td><td><img src='../images/pdf.png' width='14' height=14'>".$unite."</td>
	<td><a href='editevent.php?eventid=".$eventid."'><img src='../images/pencil.png' alt='edit' width='14' height=14'></a>
	|<a href = 'deleteevent.php?eventid=".$eventid."'><img src='../images/trash.png' width='14' height=14'></a>
	|<a href = 'virtualclass.php?unidos=".$unidos."'><img src='../images/vc2.jpg' width='14' height=14'></a></td><td></tr>";
