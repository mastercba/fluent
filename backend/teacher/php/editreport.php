...
//echo" ".$_SESSION['usuario']." ".$_SESSION['password']." ";
	...
	//Recupero variables
	$event_id2 = $_GET['eventid'];
	$_SESSION['eventid_editrep'] = $event_id2;
...
//--------------------------------recupero variables de la db a editar------------------------------------
	...
	//old trainer_report data from db
		$query44 = "select * from trainer_report"; 
		$result44 = mysql_query($query44);
		while($row44 = mysql_fetch_array($result44)){
			if($old_idstudent == $row44['idstudent'] AND $event_id2 == $row44['idevents']){
				$old_comments = $row44['comments'];
					$_SESSION['old_comments'] = $old_comments;
				$old_w_study = $row44['w_study'];
					$_SESSION['old_w_study'] = $old_w_study;
				$idreport = $row44['id'];
					$_SESSION['idreport'] = $idreport;
				$idstudentrep = $row44['idstudent'];
					$_SESSION['idstudentrep'] = $idstudentrep;
			}else{}
		}
	//cerrar
		mysql_close($conex);
...
///////AQUI pregunta para editar esa fila!/////////////////////////////////////////////////////////////
					if($event['id'] == $event_id2){
						$_SESSION['evtid']=$event['id'];
						$_SESSION['estudiante2']=$event['idstudent'];
						echo"<tr>
						<form action='saveeditrep.php' method = 'POST'>

						<td>".$snombre." ".$sapellido."</td>
						<td>".$sdate." | ".$stime."</td>
						<td><input type='text' name='comments' value='".$old_comments."' size=90px></td>
						
						<td><input type='text' name='w_study' value='".$old_w_study."' size=40px></td>
...
///////AQUI pregunta para editar esa fila!////////////////////////////////////value='".$old_idstatus."'
					if($event['id'] == $event_id2){
						$_SESSION['evtid']=$event['id'];
						$_SESSION['estudiante2']=$event['idstudent'];
						echo"<tr>
